...
func waitUntilFirewallGroupApplied(ctx context.Context, d *schema.ResourceData, svc *devops.Client) error {
	...
	err := retry.RetryContext(ctx, timeout, func() *retry.RetryError {
		...
		return retry.RetryableError(fmt.Errorf("expected the firewall group was in state APPLIED"))
	})
...
func waitUntilFirewallGroupDeleted(ctx context.Context, d *schema.ResourceData, svc *devops.Client) error {
	...
	err := retry.RetryContext(ctx, timeout, func() *retry.RetryError {
		...
		if err != nil {
			...
			return retry.RetryableError(fmt.Errorf("failed reading FirewallGroup: %s", err))
		}

		return retry.RetryableError(fmt.Errorf("expected the firewall group was deleted"))
	})
...
func waitUntilFirewallRulesRevoked(ctx context.Context, d *schema.ResourceData, svc *devops.Client, ruleIds []string) error {
	...
	err := retry.RetryContext(ctx, timeout, func() *retry.RetryError {
		...
		return retry.RetryableError(fmt.Errorf("expected rules have been revoked but were in state REVOKING"))
	})
