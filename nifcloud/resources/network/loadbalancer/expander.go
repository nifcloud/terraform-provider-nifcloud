...
func expandCreateLoadBalancerInput(d *schema.ResourceData) *computing.CreateLoadBalancerInput {
	return &computing.CreateLoadBalancerInput{
		AccountingType:   types.AccountingTypeOfCreateLoadBalancerRequest(d.Get("accounting_type").(string)),
		...
		IpVersion:     types.IpVersionOfCreateLoadBalancerRequest(d.Get("ip_version").(string)),
		PolicyType:    types.PolicyTypeOfCreateLoadBalancerRequest(d.Get("policy_type").(string)),
	}
}
...
func expandUpdateLoadBalancer(d *schema.ResourceData) *computing.UpdateLoadBalancerInput {
	...
	if d.HasChange("accounting_type") && !d.IsNewResource() {
		ac, _ := strconv.ParseInt(d.Get("accounting_type").(string), 10, 32)
		input.AccountingTypeUpdate = nifcloud.Int32(int32(ac))
	}

	if d.HasChange("network_volume") && !d.IsNewResource() {
		input.NetworkVolumeUpdate = nifcloud.Int32(int32(d.Get("network_volume").(int)))
	}
	...
	if d.HasChange("load_balancer_name") && !d.IsNewResource() {
		lbNameBefore, lbNameAfter := d.GetChange("load_balancer_name")
		input.LoadBalancerName = nifcloud.String(lbNameBefore.(string))
		input.LoadBalancerNameUpdate = nifcloud.String(lbNameAfter.(string))
	}
	return &input
}
