...
func testAccCheckELBExists(n string, elb *computing.ElasticLoadBalancerDescriptions) resource.TestCheckFunc {
...
func testAccCheckELBValues(elb *computing.ElasticLoadBalancerDescriptions, rName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		listener := elb.ElasticLoadBalancerListenerDescriptions[0].Listener

		if nifcloud.StringValue(elb.ElasticLoadBalancerName) != rName {
		...
		if nifcloud.StringValue(elb.AccountingType) != "1" {
			return fmt.Errorf("bad accounting_type state, expected \"1\", got: %#v", elb.AccountingType)
		}

		if nifcloud.StringValue(elb.NetworkVolume) != "20" {
		...
		if nifcloud.StringValue(elb.RouteTableId) == "" {
...
func testAccCheckELBValuesUpdated(elb *computing.ElasticLoadBalancerDescriptions, rName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		listener := elb.ElasticLoadBalancerListenerDescriptions[0].Listener

		if nifcloud.StringValue(elb.ElasticLoadBalancerName) != rName+"upd" {
		...
		if nifcloud.StringValue(elb.NextMonthAccountingType) != "2" {
			return fmt.Errorf("bad accounting_type state, expected \"2\", got: %#v", elb.NextMonthAccountingType)
		}

		if nifcloud.StringValue(elb.NetworkVolume) != "30" {
		...
		if nifcloud.Int64Value(listener.ElasticLoadBalancerPort) != 80 {
...
func testSweepELB(region string) error {
	...
	res, err := svc.NiftyDescribeElasticLoadBalancersRequest(nil).Send(ctx)
	if err != nil {
		return err
	}
	...
	type elb struct {
		...
		lbPort       *int64
		instancePort *int64
		protocol     computing.ProtocolOfNiftyDeleteElasticLoadBalancerRequest
	}
		...
		for _, l := range e.ElasticLoadBalancerListenerDescriptions {
			if strings.HasPrefix(nifcloud.StringValue(e.ElasticLoadBalancerName), prefix) {
				sweepELBs = append(sweepELBs, elb{
					...
					protocol:     computing.ProtocolOfNiftyDeleteElasticLoadBalancerRequest(nifcloud.StringValue(l.Listener.Protocol)),
				})
			}
		}
	}
	...
	for _, elb := range sweepELBs {
		elb := elb
		eg.Go(func() error {
			_, err := svc.NiftyDeleteElasticLoadBalancerRequest(&computing.NiftyDeleteElasticLoadBalancerInput{
