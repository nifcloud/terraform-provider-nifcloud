...
func init() {
	resource.AddTestSweepers("nifcloud_ess_domain_identity", &resource.Sweeper{
		Name: "nifcloud_ess__domain_identity",
		F:    testSweepESSDomain,
	})
}

func TestAcc_ESSDomain(t *testing.T) {
	var entry types.VerificationAttributes

	resourceName := "nifcloud_ess_domain_identity.basic"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy:      testAccESSDomainResourceDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccESSDomain(t, "testdata/ess_domain.tf", domain),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckESSDomainExists(resourceName, &entry),
					testAccCheckESSDomainValues(domain, &entry),
...
func testAccCheckESSDomainExists(n string, entry *types.VerificationAttributes) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if len(res.VerificationAttributes) == 0 {
			return fmt.Errorf("ess domain does not found in cloud: %s", saved.Primary.ID)
		}
...
func testAccCheckESSDomainValues(domain string, entry *types.VerificationAttributes) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if nifcloud.ToString(entry.Key) != domain {
			return fmt.Errorf("bad domain state, expected %s, got: %#v", domain, entry.Key)
		}
		return nil
	}
}

func testAccESSDomainResourceDestroy(s *terraform.State) error {
	svc := testAccProvider.Meta().(*client.Client).ESS

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "nifcloud_ess_domain_identity" {
			continue
		}
		...
		if len(res.VerificationAttributes) > 0 {
			return fmt.Errorf("ess domain (%s) still exists", rs.Primary.ID)
		}
	}
	return nil
}

func testSweepESSDomain(region string) error {
